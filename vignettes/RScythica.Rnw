...
Typically you receive data in a large CSV file. Before it can be used in R, the CSV file need to be transformed in a binary Scythica dataset. Scythica datasets are implements in R as S4 objects.
...
\section{Dplyr Style Pipeline}

We recommend using dplyr style commands to manipulate and transform Scythica datasets.
