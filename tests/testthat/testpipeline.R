...
test_that("Test Basic Pipeline", {
  ...
  v <- sview(sdf)
  v <- partitions(v,c("2008-01-03"))

  v <- select(v,Distance, TaxiIn)
  v <- filter(v,Distance > 2000 & TaxiIn < l)

  ds2 <- collect(v)
  
  expect_that(nrow(ds2), testthat::equals(10))  
  expect_that(sv_nrow(v), testthat::equals(10))  
...
test_that("Test Basic Pipeline - 2 ", {
  ...
  v <- select(v,Distance, TaxiIn)
  v <- filter(v,l > TaxiIn  & 2000 < Distance)
